########################################
## monogamy - model fitting (continuous)

## this script includes data preparation/coding and tests the main hypotheses using continuous measure of monogamy
## we group these hypotheses into broader theoretical concepts, which we label as "inter-group competition" and "rival wealth importance" models
...
data <- data.frame(cbind(family,soc,PP,PP_sd,AF,PI,TR,MM,SS,LP,IA,PD,AL,IC))
...
m_list <- alist(
  ...
  mu <- mu_a + z_a[family]*sigma_a + (mu_bPP + z_bPP[family]*sigma_bPP)*PP + (mu_bPI + z_bPI[family]*sigma_bPI)*PI, 
  
  # z-scores
  z_a[family] ~ normal(0,1),
  z_bPP[family] ~ normal(0,1),
  ...
  gq> vector[family]:bPP <<- mu_bPP + z_bPP*sigma_bPP,
...
af_prec <- precis(af,3,pars=c("mu_a","mu_bPP","sigma_a","sigma_phy","sigma_bPP"),prob=0.9) 
af_prec <- round(af_prec,2)
write.csv(af_prec,file="af_con_prec.csv")
af_post <- extract.samples(af)
af_post <- af_post[c(-1:-3,-11)] # remove redundant elements
write.csv(af_post,file="af_con_post.csv",row.names=FALSE)
...
m_list <- alist(
  ## trade model
  TR ~ dbinom(1,p),
  logit(p) <- mu_a + z_a[family]*sigma_a + (mu_bPP + z_bPP[family]*sigma_bPP)*PP + (mu_bPI + z_bPI[family]*sigma_bPI)*PI + phy[society],
...
## get model summary and posterior samples
tr_prec <- precis(tr,3,pars=c("mu_a","mu_bPP","sigma_a","sigma_phy","sigma_bPP"),prob=0.9) 
tr_prec <- round(tr_prec,2)
write.csv(tr_prec,file="tr_con_prec.csv")
tr_post <- extract.samples(tr)
tr_post <- tr_post[c(-1:-4,-12:-14)] # remove redundant elements
write.csv(tr_post,file="tr_con_post.csv",row.names=FALSE)
...
## get model summary and posterior samples
mm_prec <- precis(mm,3,pars=c("mu_a","mu_bPP","sigma_a","sigma_phy","sigma_bPP"),prob=0.9) 
mm_prec <- round(mm_prec,2)
write.csv(mm_prec,file="mm_con_prec.csv")
mm_post <- extract.samples(mm)
mm_post <- mm_post[c(-1:-4,-12:-14)] # remove redundant elements
write.csv(mm_post,file="mm_con_post.csv",row.names=FALSE)
...
d <- list(PP=as.numeric(data$PP),
          LP=as.numeric(data$LP),
          SS=as.integer(data$SS),
          IA=as.integer(data$IA),
          PD=as.integer(data$PD),
          AL=as.numeric(data$AL),
          IC=as.numeric(data$IC),
          family=as.integer(data$family),
          society=society,
          N=N,
          R=R
...
write.csv(lp_prec,file="lp_con_prec.csv")
lp_post <- extract.samples(lp)
lp_post <- lp_post[c(-1:-4,-9:-13,-17,-19:-21)] # remove redundant elements
write.csv(lp_post,file="lp_con_post.csv",row.names=FALSE)
...
write.csv(ss_prec,file="ss_con_prec.csv")
ss_post <- extract.samples(ss)
ss_post <- ss_post[c(-1:-5,-11:-15,-20,-22:-24)] # remove redundant elements
write.csv(ss_post,file="ss_con_post.csv",row.names=FALSE)
...
write.csv(pd_prec,file="pd_con_prec.csv")
pd_post <- extract.samples(pd)
pd_post <- pd_post[c(-1:-4,-13:-15)] # remove redundant elements
write.csv(pd_post,file="pd_con_post.csv",row.names=FALSE)
...
write.csv(al_prec,file="al_con_prec.csv")
al_post <- extract.samples(al)
al_post <- al_post[c(-1:-4,-13:-15)] # remove redundant elements
write.csv(al_post,file="al_con_post.csv",row.names=FALSE)


## estimate the total effect of incline based on the polygyny-declining model
m_list <- alist(
  # marriage patterns model
  PP ~ dbeta2(p,theta),
...
write.csv(ic_prec,file="ic_con_prec.csv")
ic_post <- extract.samples(ic)
ic_post <- ic_post[c(-1:-3,-10:-12)] # remove redundant elements
write.csv(ic_post,file="ic_con_post.csv",row.names=FALSE)
