...
RW <- apply(mono_data[,13:16],1,mean,na.rm=TRUE) # create a rival wealth proxy by averaging land ownership variables for each society
RW[is.nan(RW)] <- NA # replace NaNs with NAs
...
## before we fit the models, we'll do prior predictive simulation for the covariance function parameters
## because both parameters must be positive, we'll simulate them from an exponential distribution
## we expect relatively high phylogenetic signal of monogamy
## that is, high covariance that declines rapidly with phylogenetic and ethnographic distance
## we see how increasing eta reduces covariance, while increasing rho produces a smoother decline in covariance
## we choose eta=0.5/rho=0.5 priors, which are pretty weak and allow quite a lot of variability
n <- 1e3
eta0.5 <- rexp(n,0.5)
eta1 <- rexp(n,1)
eta2 <- rexp(n,2)
...
eta <- data.frame(cbind(eta0.5,eta0.5,eta0.5,eta1,eta1,eta1,eta2,eta2,eta2))
rho <- data.frame(cbind(rho0.5,rho1,rho2,rho0.5,rho1,rho2,rho0.5,rho1,rho2))

## plot simulated combinations of priors
priors <- c("eta=0.5 / rho=0.5","eta=0.5 / rho=1","eta=0.5 / rho=2","eta=1 / rho=0.5","eta=1 / rho=1","eta=1 / rho=2","eta=2 / rho=0.5","eta=2 / rho=1","eta=2 / rho=2")
par(mfrow=c(3,3), mar=c(5.1,5,4.1,1.1))
for(j in 1:9) {
  plot(NULL, xlim=c(0,1), ylim=c(0,5), xlab="", ylab="", main=priors[j], yaxt="n", xaxt="n", cex.main=1.75, cex.axis=1.5, axes=FALSE)
  axis(1, at=seq(from=0, to=1, length.out=5), labels=c("0.0","0.25","0.5","0.75","1.0"), cex.axis=1.5)
  axis(2, at=seq(from=0, to=5, length.out=5), labels=c("1","2","3","4","5"), cex.axis=1.5)
  mtext("covariance", 2, line=2.75, cex=1.25)
  mtext("distance", 1, line=3.15, cex=1.25)
  
  for(i in 1:100) {
    curve(eta[i,j]*exp(-rho[i,j]*x), col=col.alpha("deepskyblue3",0.2), lwd=2, add=TRUE)
  }
}
...
write.csv(data.frame(tr_prec), file="tr_prec.csv")
tr_post <- extract.samples(tr,pars=c("cutpoints","bMR","phy","ep","eta_phy","eta_ep","rho_phy","rho_ep")) 
write.csv(data.frame(tr_post), file="tr_post.csv")
...
write.csv(data.frame(tr_pi_prec), file="tr_pi_prec.csv")
tr_pi_post <- extract.samples(tr_pi,pars=c("cutpoints","bMR","bPI","phy","ep","eta_phy","eta_ep","rho_phy","rho_ep")) 
write.csv(data.frame(tr_pi_post), file="tr_pi_post.csv")
...
write.csv(data.frame(mm_prec), file="mm_prec.csv")
mm_post <- extract.samples(mm,pars=c("cutpoints","bMR","phy","ep","eta_phy","eta_ep","rho_phy","rho_ep")) 
write.csv(data.frame(mm_post), file="mm_post.csv")
...
write.csv(data.frame(mm_pi_prec), file="mm_pi_prec.csv")
mm_pi_post <- extract.samples(mm_pi,pars=c("cutpoints","bMR","bPI","phy","ep","eta_phy","eta_ep","rho_phy","rho_ep")) 
write.csv(data.frame(mm_pi_post), file="mm_pi_post.csv")
...
write.csv(data.frame(mc_prec), file="mc_prec.csv")
mc_post <- extract.samples(mc,pars=c("a","bMR","sigma","eta_phy","eta_ep","rho_phy","rho_ep")) 
write.csv(data.frame(mc_post), file="mc_post.csv")
...
write.csv(data.frame(mc_pi_prec), file="mc_pi_prec.csv")
mc_pi_post <- extract.samples(mc_pi,pars=c("a","bMR","bPI","sigma","phy","ep","eta_phy","eta_ep","rho_phy","rho_ep")) 
write.csv(data.frame(mc_pi_post), file="mc_pi_post.csv")
...
m_list <- alist(
  ...
  # imputation model for rival wealth modeled
  RW ~ normal(mu,sigma),
  mu <- aRW + bIA_RW*IA + bPD_RW*PD + bDC_RW*DC + bAL_RW*AL + bIC_RW*IC,
...
## get model summary and posterior samples
rw_prec <- precis(rw,2,pars=c("cutpoints","bRW","bIA","bPD","bDC","bAL","bIC","bIA_RW","bPD_RW","bDC_RW","bAL_RW","bIC_RW","sigma","eta_phy","eta_ep","rho_phy","rho_ep"))
rw_prec <- data.frame(round(rw_prec,2))
write.csv(data.frame(rw_prec), file="rw_prec.csv")
rw_post <- extract.samples(rw,pars=c("cutpoints","bRW","bIA","bPD","bDC","bAL","bIC","bIA_RW","bPD_RW","bDC_RW","bAL_RW","bIC_RW","sigma","eta_phy","eta_ep","rho_phy","rho_ep")) 
write.csv(data.frame(rw_post), file="rw_post.csv")


## fit the models for land shortage of Fortunato and Archetti (2010)
...
write.csv(data.frame(pd_prec), file="pd_prec.csv")
pd_post <- extract.samples(pd,pars=c("cutpoints","bPD","bIA","eta_phy","eta_ep","rho_phy","rho_ep")) 
write.csv(data.frame(pd_post), file="pd_post.csv")
...
write.csv(data.frame(dc_prec), file="dc_prec.csv")
dc_post <- extract.samples(dc,pars=c("cutpoints","bDC","eta_phy","eta_ep","rho_phy","rho_ep")) 
write.csv(data.frame(dc_post), file="dc_post.csv")
...
write.csv(data.frame(ai_prec), file="ai_prec.csv")
ai_post <- extract.samples(ai,pars=c("cutpoints","bAL","bIC","eta_phy","eta_ep","rho_phy","rho_ep")) 
write.csv(data.frame(ai_post), file="ai_post.csv")
...
write.csv(data.frame(vn_prec), file="vn_prec.csv")
vn_post <- extract.samples(vn,pars=c("cutpoints","bVN","bVI","phy","ep","eta_phy","eta_ep","rho_phy","rho_ep")) 
write.csv(data.frame(vn_post), file="vn_post.csv")
